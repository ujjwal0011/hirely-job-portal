...
const Register = () => {
  ...
  const { loading, isAuthenticated, error, isVerified, message } = useSelector(
    (state) => state.user
  );
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && !isVerified) {
      navigateTo(`/verify-email/${formData.email}?action=verify`);
    }
  }, [dispatch, error, isAuthenticated, navigateTo]);

  return (
    <div className="container mx-auto flex min-h-screen px-4 py-12 mt-14">
                      ...
                      {["firstNiche", "secondNiche", "thirdNiche"].map(
                        (nicheField, idx) => (
                          <div key={idx} className="space-y-2">
                            <Label htmlFor={nicheField}>{`Your ${idx + 1}${
                              idx === 0 ? "st" : idx === 1 ? "nd" : "rd"
                            } Niche`}</Label>
                            <Select
                              value={formData[nicheField]}
                              onValueChange={(value) =>
                                handleSelectChange(nicheField, value)
                              }
                            >
